import * as z from "zod";
import { Result as SafeParseResult } from "../../types/fp.js";
import { SDKValidationError } from "../errors/sdkvalidationerror.js";
import { ProductPriceRecurringCustom, ProductPriceRecurringCustom$Outbound } from "./productpricerecurringcustom.js";
import { ProductPriceRecurringFixed, ProductPriceRecurringFixed$Outbound } from "./productpricerecurringfixed.js";
import { ProductPriceRecurringFree, ProductPriceRecurringFree$Outbound } from "./productpricerecurringfree.js";
export type ProductPriceRecurring = ProductPriceRecurringFree | ProductPriceRecurringFixed | ProductPriceRecurringCustom;
/** @internal */
export declare const ProductPriceRecurring$inboundSchema: z.ZodType<ProductPriceRecurring, z.ZodTypeDef, unknown>;
/** @internal */
export type ProductPriceRecurring$Outbound = ProductPriceRecurringFree$Outbound | ProductPriceRecurringFixed$Outbound | ProductPriceRecurringCustom$Outbound;
/** @internal */
export declare const ProductPriceRecurring$outboundSchema: z.ZodType<ProductPriceRecurring$Outbound, z.ZodTypeDef, ProductPriceRecurring>;
/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export declare namespace ProductPriceRecurring$ {
    /** @deprecated use `ProductPriceRecurring$inboundSchema` instead. */
    const inboundSchema: z.ZodType<ProductPriceRecurring, z.ZodTypeDef, unknown>;
    /** @deprecated use `ProductPriceRecurring$outboundSchema` instead. */
    const outboundSchema: z.ZodType<ProductPriceRecurring$Outbound, z.ZodTypeDef, ProductPriceRecurring>;
    /** @deprecated use `ProductPriceRecurring$Outbound` instead. */
    type Outbound = ProductPriceRecurring$Outbound;
}
export declare function productPriceRecurringToJSON(productPriceRecurring: ProductPriceRecurring): string;
export declare function productPriceRecurringFromJSON(jsonString: string): SafeParseResult<ProductPriceRecurring, SDKValidationError>;
//# sourceMappingURL=productpricerecurring.d.ts.map